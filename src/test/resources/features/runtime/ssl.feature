@ssl
Feature: Detect changes in the established SSL baseline - csaa-insurance

  Background: Run the SSLyze command only once for all features
    When the SSLyze command is run against the host csaa-insurance.aaa.com on port 443
  ...
  Scenario: Disable client renegotiations
    Then the output must contain a line that matches .*VULNERABLE - Server honors client-initiated renegotiations.*

  Scenario: Disable downgrade attacks
    Then the output must not contain a line that matches .*VULNERABLE - Signaling cipher suite not supported.*

  @iriusrisk-ssl_heartbleed
  Scenario: Patch OpenSSL against the Heartbleed vulnerability
    Then the output must contain a line that matches .*Unhandled exception while running --heartbleed.*

  @iriusrisk-ssl_strong_cipher
  Scenario: The minimum cipher strength should meet requirements
    Then the minimum key size must be 112 bits
