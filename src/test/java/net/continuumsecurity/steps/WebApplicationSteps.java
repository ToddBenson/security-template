...
//            proxy.setAttackMode();
    ...
    @Then("the HTTP (.*) header is not present")
    public void checkHeaderValue(String name) {
        HarEntry currentHar = World.getInstance().getCurrentHar();
        assertNull("No HTTP header named: " + name + " was found.", Utils.getResponseHeaderValue(currentHar.getResponse(), name));
    }
