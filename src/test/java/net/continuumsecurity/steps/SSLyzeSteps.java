...
public class SSLyzeSteps {
    ...
    @Then("the output must not contain the text (.*)")
    public void verifyThatOutputNotContainsText(String text) throws IOException {
        if (text.startsWith("\"") || text.startsWith("'")) text = text.substring(1, text.length() - 1);
        assertThat(World.getInstance().getjSSLyze().getOutput(), not(containsString(text)));
    }
    ...
    @Then("^the output must not contain a line that matches (.*)")
    public void verifyThatOutputNotMatchesRegex(String regex) throws IOException {
        if (regex.startsWith("\"") || regex.startsWith("'")) regex = regex.substring(1, regex.length() - 1);
        assertThat(World.getInstance().getjSSLyze().getParser().doesAnyLineMatch(regex), not(equalTo(true)));
    }
